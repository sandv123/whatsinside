# LLM-powered RAG-driven local directory inspection service
...
## Architecture
I will use a local LLM (small one to fit into my 4Gb GF3050 mobile GPU) to summarize the documents, index them, store the embeddings in a local vector DB. Upon a user query the service will check if all files in the directory have been indexed, perform cluster analyzis on the embedding vectors, corresponding to the files in the directory, for every cluster identified a summary will be generated to describe the nature of the documents, which will be used to respond to the user query.